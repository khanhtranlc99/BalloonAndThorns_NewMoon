...
using UnityEngine;
using DG.Tweening;
public class ElectricBall : BallBase
{
            ...
            foreach (var hit in hits)
            {
                if (hit.collider != null && !hit.collider.isTrigger)
                {
                    ...
                    if (hit.collider.gameObject.GetComponent<BarrialAir>() != null)
                    {
                        ...
                        SimplePool2.Spawn(vfxElectric, hit.collider.gameObject.transform.position, Quaternion.identity);
                        foreach(var item in hit.collider.gameObject.GetComponent<BarrialAir>().GetBallondsAround())
                        {
                            
                                SimplePool2.Spawn(vfxElectric, item.gameObject.transform.position, Quaternion.identity);
                                item.TakeDameSpike(DamePlusCard.dame);
